...
return {
  ...
  --
  -- registry = { index = "https://github.com/rust-lang/crates.io-index" }, -- newly added. to overwrite resistory.
  ...
  config = function(_, opts) -- <-- config is a sibling, not inside opts
    ...
    vim.keymap.set("n", "<leader>Ci", crates.show_popup, { desc = "ℹ️ Show crate info" })
    ...
    vim.keymap.set("v", "<leader>Cx", crates.upgrade_crates, { desc = "⚡ UUpgrade selected crates" })
    vim.keymap.set("n", "<leader>CX", crates.upgrade_all_crates, { desc = "⚡ UUpgrade ALL crates" })
